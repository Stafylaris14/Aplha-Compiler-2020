...
avm_memcell *avm_tablegetelem(avm_table *table ,avm_memcell *key)//diale3i 13 oxi ilopoio
{
 printf("avm table get\n");
  ...
  switch(key->type)
  {
    case number_m:
     ...
     avm_memcell *tmp = malloc(sizeof(avm_memcell));
     tmp->data = table->numIndexed[index]->value;
    ...
    case string_m:
       index = hash_int(key->data.strVal);
          avm_memcell *tmp = malloc(sizeof(avm_memcell));
          tmp->data = table->strIndexed[index]->value;
       return tmp;
...
void avm_tablesetelem(avm_table *table, avm_memcell *key, avm_memcell *val) //dial 13 oxi ioloipoio
{
  printf("avm table set\n");
  //den 3erw ti fasei prepei na to doume
  int index = 0;
  switch(key->type)
  {

...
int accessToFunction(item *new)
{
  ...
  if (proigoumeno != NULL)
  {
    ...
    if (twra != NULL)
    {
      if (!strcmp(twra->type, "local variable"))
        return 1;
    }
  ...
  else if (glob != NULL)
  {
    ...
    {
      if (twra != NULL)
        if (!strcmp(twra->type, "formal argument"))
          return 1;
...
void new_check(item *new)
{
  ...
  for (i = 0; i < HASH_SIZE; i++)
  {
    ...
    while (tmp != NULL)
    {
      if (!strcmp(tmp->name, new->name) && tmp->isActive)
      { //an einai idio onoma meta 3ekinaw elenxoys
